...
function computeStreak(logs: { logged_at: string; completed: boolean }[]): {
  ...
  const completedDates = new Set(
    logs.filter(l => l.completed).map(l => l.logged_at.split("T")[0])
  );
  ...
  try {
    ...
    const habitStreaks: HabitStreak[] = (habitsResult.data ?? []).map(habit => {
      ...
      return {
        ...
        completion_rate_30d: logs.length > 0 ? Math.round((completedCount / 30) * 100) : 0,
      };
    });
    ...
    const expenses = txns.filter(t => t.type === "expense").reduce((s, t) => s + Math.abs(t.amount ?? 0), 0);
    const financeStats = {
