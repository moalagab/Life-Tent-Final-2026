...
import {
  ...
  TrendingUp, TrendingDown, Minus, Plus, Loader2,
} from 'lucide-react';
...
function PlanStep({ plan }: { plan: ReturnType<typeof useDailyPlanningCycle>['plan'] }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      await createTask.mutateAsync({ title: newTitle.trim(), status: 'todo', due_date: today });
      toast.success('تم إضافة المهمة لليوم');
      ...
      {addingTask ? (
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={e => { if (e.key === 'Enter') handleAdd(); if (e.key === 'Escape') { setAddingTask(false); setNewTitle(''); } }}
          ...
          />
          <button onClick={handleAdd} disabled={createTask.isPending || !newTitle.trim()}
            className="px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-medium disabled:opacity-50">
            {createTask.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : '+ أضف'}
          </button>
          <button onClick={() => { setAddingTask(false); setNewTitle(''); }}
            className="px-2 py-1.5 rounded-lg bg-muted text-muted-foreground text-xs">
            <X className="w-3 h-3" />
          </button>
        </div>
      ) : (
