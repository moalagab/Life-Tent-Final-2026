...
import { ProjectHealthDashboard } from '@/components/modules/projects/ProjectHealthDashboard';
import { PhaseIntelligenceBar }   from '@/components/modules/projects/PhaseIntelligenceBar';
import { ActivityFeed }           from '@/components/modules/projects/ActivityFeed';
import type { ProjectPhase, PhaseProgress, RiskCount } from '@/types/projects.types';
...
export default function ProjectWorkspace() {
  ...
  // Health engine data
  const { project: enrichedProject, activity, loading: healthLoading } = useProject(id ?? '');
  ...
  const overdueTasks = projectTasks.filter(t =>
    t.due_at && !['done', 'completed'].includes(t.status ?? '') && new Date(t.due_at) < new Date()
  ).length;
  ...
  // Merge enriched health data with base project
  const healthData = enrichedProject ?? null;
  const phaseProgressData: PhaseProgress = (healthData?.phase_progress) ?? {
    initiation: 0, planning: 0, execution: 0, monitoring: 0, closing: 0,
  };
  const riskCountData: RiskCount = (healthData?.risk_count) ?? { high: 0, medium: 0, low: 0 };
        ...
        {healthData && (
          <div className="glass-card p-4">
            <ProjectHealthDashboard
              ...
              onRisksClick={() => setActiveTab('overview')}
            />
          </div>
        )}

        {/* ── Phase Intelligence Bar ── */}
        <div className="glass-card p-4">
          <p className="text-xs text-muted-foreground mb-2 font-medium">{isAr ? 'مراحل المشروع (PMBOK)' : 'Project Phases (PMBOK)'}</p>
          <PhaseIntelligenceBar
            currentPhase={(project.phase ?? 'initiation') as ProjectPhase}
            phaseProgress={phaseProgressData}
            onPhaseClick={(phase) => handlePhaseChange(phase)}
          />
        </div>
          ...
          {activeTab === 'overview' && (
            <div className="space-y-4">
              ...
              <div className="glass-card p-4">
                ...
                <ActivityFeed events={activity} loading={healthLoading} />
              </div>
