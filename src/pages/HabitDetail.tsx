    ...
    return (
      <MainLayout>
        <div className="flex flex-col items-center justify-center h-64 gap-3">
          ...
          <p className="text-muted-foreground">العادة غير موجودة</p>
          <Button variant="outline" onClick={() => navigate('/habits')}>العودة للعادات</Button>
        </div>
      </MainLayout>
    );
  }

  if (!habit) return <MainLayout><div className="flex items-center justify-center h-64"><Loader2 className="w-6 h-6 animate-spin text-muted-foreground" /></div></MainLayout>;
    ...
    <MainLayout>
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <button onClick={() => navigate('/habits')} className="hover:text-foreground transition-colors">العادات</button>
        ...
        <div className="glass-card p-5 relative overflow-hidden" style={{ borderTop: `3px solid ${habitColor}` }}>
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <Badge variant="outline" className="text-xs">{FREQ_LABELS[habit.frequency ?? 'daily']}</Badge>
                {habit.target_count && <span className="text-xs text-muted-foreground">الهدف: {habit.target_count}x</span>}
              </div>
              {habit.description && <p className="text-sm text-muted-foreground mt-1.5 line-clamp-2">{habit.description}</p>}
            </div>
          </div>
          ...
          <div className="relative grid grid-cols-3 gap-3 mt-5">
            {[
              { label: 'السلسلة', value: `${currentStreak}`, sub: 'يوم متواصل', color: 'text-warning' },
              { label: 'الإنجاز', value: `${completionRate}%`, sub: 'آخر 30 يوم', color: 'text-success' },
              { label: 'الإجمالي', value: logs.length, sub: 'تسجيل', color: 'text-primary' },
            ].map(s => (
        ...
        <div className="glass-card p-4 space-y-3">
          ...
          <div className="grid grid-cols-10 gap-1.5">
            {last30.map((d, i) => (
              <div
                key={i}
                title={format(d.date, 'dd MMM', { locale: ar })}
        ...
        <div className="glass-card p-4 space-y-3">
          ...
          {logs.slice(0, 10).map(log => (
            <div key={log.id} className="flex items-center gap-2.5 p-2 rounded-xl bg-muted/20">
              ...
              <span className="text-sm flex-1">{log.completed_at ? format(new Date(log.completed_at), 'EEEE، dd MMM yyyy', { locale: ar }) : '—'}</span>
            </div>
          ))}
          {logs.length === 0 && <p className="text-xs text-muted-foreground text-center py-4">لا توجد سجلات بعد</p>}
        </div>
            ...
            {Array.from({ length: 4 }, (_, weekIdx) => {
              ...
              return (
                <div key={weekIdx} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-md transition-all" style={{ height: `${height}%`, minHeight: 4, backgroundColor: height > 0 ? habitColor : undefined, opacity: height > 0 ? 1 : 0.2 }} />
                  <span className="text-[9px] text-muted-foreground">أ{weekIdx + 1}</span>
                </div>
              );
            })}
          </div>
